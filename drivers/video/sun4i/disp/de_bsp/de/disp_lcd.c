...
void LCD_get_sys_config(__u32 sel, __disp_lcd_cfg_t * lcd_cfg)
{
	char io_name[28][20] = {
		...
		"lcdd16", "lcdd17", "lcdd18", "lcdd19",
		"lcdd20", "lcdd21", "lcdd22", "lcdd23",
...
//lcd_bl_en
	...
	if (value == 0) {
		...
		ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_bl_en",
						   (int *)gpio_info,
...
//lcd_power
	...
	if (value == 0) {
		...
		ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_power",
						   (int *)gpio_info,
						   sizeof(user_gpio_set_t)/sizeof(int));
		if (ret < 0) {
...
//lcd_pwm
	...
	if (value == 0) {
		...
		ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_pwm",
						   (int *)gpio_info,
						   sizeof(user_gpio_set_t)/sizeof(int));
		if (ret < 0) {
...
//lcd_gpio
	for (i = 0; i < 4; i++) {
		...
		ret = OSAL_Script_FetchParser_Data(primary_key, sub_name, (int *)gpio_info,
						 sizeof(user_gpio_set_t)/sizeof(int));
		if (ret < 0) {
...
//lcd io
	for (i = 0; i < 28; i++) {
		...
		ret = OSAL_Script_FetchParser_Data(primary_key, io_name[i],
						   (int *)gpio_info,
						   sizeof(user_gpio_set_t)/sizeof(int));
		if (ret < 0) {
