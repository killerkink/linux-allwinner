...
static __s32 DRV_lcd_close(__u32 sel)
{
	__u32 i = 0;
	__lcd_flow_t *flow;
...
static __s32 disp_set_hdmi_func(__disp_hdmi_func * func)
{
...
static __s32 DRV_DISP_Init(void)
{
...
static __s32 DRV_DISP_Exit(void)
{
	Fb_Exit();
	BSP_disp_close();
...
static ssize_t disp_read(struct file *file, char __user *buf, size_t count,
			 loff_t* ppos)
{
	return 0;
}

static ssize_t disp_write(struct file *file, const char __user *buf,
...
static long disp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd != DISP_CMD_TV_GET_INTERFACE &&
	    ...
	    cmd != DISP_CMD_GET_OUTPUT_TYPE &&
	    cmd != DISP_CMD_SCN_GET_WIDTH &&
	    cmd != DISP_CMD_SCN_GET_HEIGHT &&
	...
	case DISP_CMD_SPRITE_SET_FORMAT:
		ret = BSP_disp_sprite_set_format(ubuffer[0], (__disp_pixel_fmt_t)ubuffer[1],
