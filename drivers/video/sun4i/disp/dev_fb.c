...
static __s32 fb_draw_colorbar(__u32 base, __u32 width, __u32 height, struct fb_var_screeninfo * var)
{
	__u32 i = 0, j = 0;
		...
		for (j = 0; j < width / 4; j++) {
			...
			if (var->bits_per_pixel == 32) {
				...
				sys_put_wvalue(base + offset * 4,
					       (((1 << var->transp.length) - 1) << var->transp.offset) |
					       (((1 << var->red.length) - 1) << var->red.offset));

				offset = width * i + j + width / 4;
				sys_put_wvalue(base + offset * 4,
					       (((1 << var->transp.length) - 1) << var->transp.offset) |
					       (((1 << var->green.length) - 1) << var->green.offset));

				offset = width * i + j + width / 4 * 2;
				sys_put_wvalue(base + offset * 4,
					       (((1 << var->transp.length) - 1) << var->transp.offset) |
					       (((1 << var->blue.length) - 1) << var->blue.offset));
...
static __s32 fb_draw_gray_pictures(__u32 base, __u32 width, __u32 height,
		...
		case 16:
			if (var->red.length == 6 && var->green.length == 5 && var->blue.length == 5) {
				...
				if (var->transp.offset == 15 &&
				    var->red.offset == 10 && var->green.offset == 5 && var->blue.offset == 0) { /*argb*/
					var->reserved[2] = DISP_SEQ_ARGB;
					var->reserved[3] = 0;
				} else if (var->transp.offset == 15 &&
					   var->blue.offset == 10 && var->green.offset == 5 && var->red.offset == 0) { /*abgr*/
					var->reserved[2] = DISP_SEQ_ARGB;
					var->reserved[3] = 1;
				} else {
				...
				if (var->transp.offset == 12 &&
				    var->red.offset == 8 && var->green.offset == 4 && var->blue.offset == 0) { /*argb*/
					var->reserved[2] = DISP_SEQ_ARGB;
					var->reserved[3] = 0;
				} else if (var->transp.offset == 12 &&
					   var->blue.offset == 8 && var->green.offset == 4 && var->red.offset == 0) { /*abgr*/
					var->reserved[2] = DISP_SEQ_ARGB;
					var->reserved[3] = 1;
				} else {
...
__s32 Fb_Init(__u32 from)
{
	...
	if (g_fbi.disp_init.b_init) {
		...
		for (i = 0; i < fb_num; i++) {
			...
			disp_fb_to_var(g_fbi.disp_init.format[i], g_fbi.disp_init.seq[i],
				       g_fbi.disp_init.br_swap[i], &(g_fbi.fbinfo[i]->var));
		...
		if (g_fbi.disp_init.disp_mode != DISP_INIT_MODE_SCREEN1) {
			...
			if ((g_fbi.disp_init.output_type[0] == DISP_OUTPUT_TYPE_TV) ||
			    (g_fbi.disp_init.output_type[0] == DISP_OUTPUT_TYPE_VGA)) {
		...
		if (g_fbi.disp_init.disp_mode != DISP_INIT_MODE_SCREEN0) {
			...
			if ((g_fbi.disp_init.output_type[1] == DISP_OUTPUT_TYPE_TV) ||
			    (g_fbi.disp_init.output_type[1] == DISP_OUTPUT_TYPE_VGA)) {
