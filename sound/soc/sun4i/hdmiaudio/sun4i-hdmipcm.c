...
static int __devinit sun4i_soc_platform_hdmiaudio_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &sun4i_soc_platform_hdmiaudio);
}

static int __devexit sun4i_soc_platform_hdmiaudio_remove(struct platform_device *pdev)
{
	snd_soc_unregister_platform(&pdev->dev);
	return 0;
}

static struct platform_driver sun4i_soc_platform_hdmiaudio_driver = {
	.driver = {
		.name = "sun4i-audio",
		.owner = THIS_MODULE,
	},

	.probe = sun4i_soc_platform_hdmiaudio_probe,
	.remove = __devexit_p(sun4i_soc_platform_hdmiaudio_remove),
};

static int __init sun4i_soc_platform_hdmiaudio_init(void)
{
	return platform_driver_register(&sun4i_soc_platform_hdmiaudio_driver);
}
module_init(sun4i_soc_platform_hdmiaudio_init);

static void __exit sun4i_soc_platform_hdmiaudio_exit(void)
{
	platform_driver_unregister(&sun4i_soc_platform_hdmiaudio_driver);
}
